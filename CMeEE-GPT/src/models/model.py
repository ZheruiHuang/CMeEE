...
from src.utils.const import EE_label2id1, NER_PAD
...
class LinearClassifier(nn.Module):
    def __init__(self, hidden_size: int, num_labels: int, dropout: float):
        ...
        self.layers = nn.Sequential(nn.Dropout(dropout), nn.Linear(hidden_size, num_labels))
    ...
    def forward(self, hidden_states, labels=None, no_decode=False):
        ...
        return NEROutputs(loss, pred_labels)  # type: ignore
...
class CRFClassifier(nn.Module):
    ...
    def _pred_labels(self, _logits, mask, label_pad_tag):
        batch_size, seq_len, num_labels = _logits.shape
        pred_labels = self.crf.decode(_logits, mask=mask)  # List[List[int]]
        assert len(pred_labels) == batch_size
        for pred_label in pred_labels:
            pred_label.extend([label_pad_tag] * (seq_len - len(pred_label)))
        pred_labels = torch.tensor(pred_labels, device=_logits.device)
        return pred_labels
    ...
    def forward(self, hidden_states, attention_mask, labels=None, no_decode=False, label_pad_token_id=NER_PAD_ID):
        ...
        if labels is None:
            pred_labels = self._pred_labels(_logits, mask=attention_mask.bool(), label_pad_tag=label_pad_token_id)
        else:
            loss = -self.crf(_logits, labels, mask=attention_mask.bool(), reduction="mean")  # default reduction: sum
            if not no_decode:
                pred_labels = self._pred_labels(_logits, mask=attention_mask.bool(), label_pad_tag=label_pad_token_id)
...
def _group_ner_outputs(output1: NEROutputs, output2: NEROutputs):
    ...
    return NEROutputs(grouped_loss, grouped_logits)  # type: ignore
