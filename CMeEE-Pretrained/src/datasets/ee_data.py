...
class EEDataloader:
    ...
    def get_data(self, mode: str, synonyms_replacement: bool):
        if mode not in ("train", "dev", "test"):
            raise ValueError(f"Unrecognized mode: {mode}")
        if mode in ("dev", "test") and synonyms_replacement:
            raise ValueError(f"synonyms_replacement is not supported in {mode} mode")
        if mode in ("train",) and synonyms_replacement and not exists(join(self.data_root, f"CMeEE_{mode}_synonyms.json")):
            raise RuntimeError(f"Please run `python src/datasets/synonyms_replacement.py` to generate CMeEE_{mode}_synonyms.json first")
        json_filename = join(self.data_root, f"CMeEE_{mode}{'_synonyms' if synonyms_replacement else ''}.json")
        print(f"Loading {json_filename} as {mode} set...")
        return self._parse(self._load_json(json_filename))


class EEDataset(Dataset):
    def __init__(self, cblue_root: str, mode: str, max_length: int, tokenizer, for_nested_ner: bool, synonyms_replacement: bool = False):
        self.cblue_root = cblue_root
        self.data_root = join(cblue_root, "CMeEE")
        ...
        if False and exists(cache_file):
            ...
            self.examples = EEDataloader(cblue_root).get_data(mode, synonyms_replacement) # get original data
...
class CollateFnForEE:
    ...
    def __call__(self, batch) -> dict:
        ...
        if not self.for_nested_ner:
            ...
            labels1 = [x[1]  for x in inputs] if len(inputs[0]) > 1 else None
...
if __name__ == '__main__':
    ...
    CBLUE_ROOT = os.path.join(pwd, "../../data/CBLUEDatasets")
    for_nested_ner = True  # for testing

    tokenizer = BertTokenizer.from_pretrained(MODEL_NAME)
    dataset = EEDataset(CBLUE_ROOT, mode="dev", max_length=10, tokenizer=tokenizer, for_nested_ner=for_nested_ner)

    batch = [dataset[0], dataset[1], dataset[2]]
    inputs = CollateFnForEE(pad_token_id=tokenizer.pad_token_id, for_nested_ner=for_nested_ner)(batch)
    print(inputs)
